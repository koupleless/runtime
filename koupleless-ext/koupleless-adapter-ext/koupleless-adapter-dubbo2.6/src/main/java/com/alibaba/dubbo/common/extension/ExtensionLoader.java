...
public class ExtensionLoader<T> {
    ...
    public static <T> ExtensionLoader<T> getExtensionLoader(Class<T> type) {
        ...
        if (classExtensionLoaderConcurrentMap == null) {
            EXTENSION_LOADERS_SUPPORT_CLASSLOADER.putIfAbsent(classLoader,
                new ConcurrentHashMap<>());
            classExtensionLoaderConcurrentMap = EXTENSION_LOADERS_SUPPORT_CLASSLOADER
                .get(classLoader);
            //TO avoid static init only once like ReferenceConfig
        ...
        //
        ExtensionLoader<T> loader = (ExtensionLoader<T>) classExtensionLoaderConcurrentMap
            .get(type);
        if (loader == null) {
            classExtensionLoaderConcurrentMap.putIfAbsent(type, new ExtensionLoader<T>(type));
    ...
    public void addExtension(String name, Class<?> clazz) {
        ...
        if (!type.isAssignableFrom(clazz)) {
            throw new IllegalStateException(
                "Input type " + clazz + "not implement Extension " + type);
        }
        ...
        if (!clazz.isAnnotationPresent(Adaptive.class)) {
            ...
            if (cachedClasses.get().containsKey(name)) {
                throw new IllegalStateException(
                    "Extension name " + name + " already existed(Extension " + type + ")!");
            }
    ...
    public void replaceExtension(String name, Class<?> clazz) {
        ...
        if (!clazz.isAnnotationPresent(Adaptive.class)) {
            ...
            if (!cachedClasses.get().containsKey(name)) {
                throw new IllegalStateException(
                    "Extension name " + name + " not existed(Extension " + type + ")!");
            }
                ...
                synchronized (cachedAdaptiveInstance) {
                    ...
                    if (instance == null) {
                        try {
                            ...
                            throw new IllegalStateException(
                                "fail to create adaptive instance: " + t.toString(), t);
                        }
                    }
                }
            } else {
                ...
                for (Method method : instance.getClass().getMethods()) {
                    if (method.getName().startsWith("set") && method.getParameterTypes().length == 1
                        ...
                        try {
                            ...
                            logger.error(
                                "fail to inject via method " + method.getName() + " of interface "
    ...
    private Class<?> getExtensionClass(String name) {
        ...
        if (clazz == null)
            throw new IllegalStateException(
                "No such extension \"" + name + "\" for " + type.getName() + "!");
        return clazz;
    }
    ...
    private void loadResource(Map<String, Class<?>> extensionClasses, ClassLoader classLoader,
        ...
        try {
            ...
            logger.error("Exception when load extension class(interface: " + type + ", class file: "
    ...
    private void loadClass(Map<String, Class<?>> extensionClasses, java.net.URL resourceURL,
        ...
        if (clazz.isAnnotationPresent(Adaptive.class)) {
            if (cachedAdaptiveClass == null) {
                ...
                throw new IllegalStateException(
                    "More than 1 adaptive class found: " + cachedAdaptiveClass.getClass().getName()
            ...
            if (name == null || name.length() == 0) {
                ...
                if (name.length() == 0) {
                    throw new IllegalStateException(
                        "No such extension name for the class " + clazz.getName()
    ...
    private String findAnnotationName(Class<?> clazz) {
        com.alibaba.dubbo.common.Extension extension = clazz
            .getAnnotation(com.alibaba.dubbo.common.Extension.class);
        if (extension == null) {
        ...
        for (Method method : methods) {
            ...
            if (adaptiveAnnotation == null) {
                code.append("throw new UnsupportedOperationException(\"method ")
                    .append(method.toString()).append(" of interface ").append(type.getName())
                ...
                // did not find parameter in URL type
                    ...
                    if (attribMethod == null) {
                        throw new IllegalStateException(
                            "fail to create adaptive class for interface " + type
                                .getName() + ": not found url parameter or url attribute in parameters of method "
                                                        + method.getName());
                    }
